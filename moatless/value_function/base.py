...
from moatless.actions.verified_finish import VerifiedFinishArgs
...
class ValueFunction2(BaseModel):
    ...
    def get_reward(self, node: Node) -> Tuple[Reward, Optional[Completion]]:
        ...
        def get_action_id(action_steps: list):
            ...
            try:
                if type(action_steps[0].action) == RejectArgs:
                    return 0
                elif type(action_steps[0].action) == SemanticSearchArgs:
                    return 1
                elif type(action_steps[0].action) == ViewCodeArgs: 
                    return 2
                elif type(action_steps[0].action) == FindClassArgs:
                    return 3
                elif type(action_steps[0].action) == FindCodeSnippetArgs:
                    return 4
                elif type(action_steps[0].action) == CreateFileArgs:
                    return 5
                elif type(action_steps[0].action) == FindFunctionArgs:
                    return 6
                elif type(action_steps[0].action) == RunTestsArgs:
                    return 7
                elif type(action_steps[0].action) == VerifiedFinishArgs:
                    return 8
                elif type(action_steps[0].action) == StringReplaceArgs:
                    return 9
                else:
                    return 0
            except:
        ...
        try:
            ...
            try:
                ...
                if not node.parent:
                    ...
                    state[3 : 3 + 384] = sentence_embedding_model.encode(get_prompt(node))
                    ...
                    state[3] = int(completion_response.structured_output.value)
                    state[3 : 3 + 384] = sentence_embedding_model.encode(get_prompt(node))
                    state[3 + 384 : 3 + 384 * 2] = sentence_embedding_model.encode(
                ...
                with torch.no_grad():
                    ...
                    reward = int(1000*(value_function_model(x).numpy()))

                completion_response.structured_output.value = reward
                print(f'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ reward: {reward}')
            
            except:
                print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Error in new reward function!!!!!!!')
